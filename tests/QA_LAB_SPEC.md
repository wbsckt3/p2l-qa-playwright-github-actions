...
| `qa-lab/momentum-1.accepted.spec.js` | Runner por etapas (`runUntil`) y validación explícita de **Momentum 1** hasta `accepted`. Permite ejecutar por stage con `P2L_STAGE_TARGET` (`M1_REQUESTED`, `M1_ACCEPTED`, `M2_ARRIVED`, `M3_IN_PROGRESS`, `M4_COMPLETED`). **Por defecto exige solicitud real** (pasajero real). Fallback sintético solo si se define `P2L_ALLOW_SYNTHETIC=1`. |
