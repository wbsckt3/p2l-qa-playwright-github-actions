...
| `qa-lab/momentums/m1-to-accepted.spec.js` … `m4-to-completed.spec.js` | **Un spec = un momentum**: mismo orden que docs en repo producto (`momentum 1`…`momentum 4` · txt en raíz uber-like). `runUntil` en `stageRunner.js` (+ pickup→modal antes de `arrived`). Scripts: `npm run test:qa:m1` … `test:qa:m4`, o `test:qa:momentums`. **Solicitud real por defecto**; `P2L_ALLOW_SYNTHETIC=1` solo para depuración. |
